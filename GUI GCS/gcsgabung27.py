...
print(url)
handle = urlopen(url)
read_data_photo = handle.read()
imgdata_photo = Image.open(BytesIO(read_data_photo))
#imgdata_photo = base64.encodestring(read_data_photo)
handle.close()
...
data_photo = ImageTk.PhotoImage(imgdata_photo)
...
lmain_map = tk.Label(image = data_photo)
...
def show_frame():
    ...
    if ((xtile != xtile1) | (ytile != ytile1)):
        ...
        #with urlopen(url) as handle:
        handle = urlopen(url)
        readimgdata_photo = handle.read()
        imgdata_photo = Image.open(BytesIO(readimgdata_photo))
        data_photo = ImageTk.PhotoImage(imgdata_photo)
        handle.close()

        #imgg = tk.PhotoImage(data=data_photo)
        #lmain_map.imgtk = imgg
        lmain_map.imgtk = data_photo
        lmain_map.configure(image = data_photo)
