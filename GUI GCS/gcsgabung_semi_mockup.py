...
latl = -7.2831759 #koordinat pakuwon
lonl = 112.8115028  #koordinat pakuwon
z = 15  # Zoom level

lat_target = latl   # inisialisasi lokasi target
lon_target = lonl
global jarak_x
jarak_x = 0
...
Sudut = 43.0 #Tergantung kemiringan pesawat

bearing = 0.0#arah pesawat utara (0) derajat

# mengira-ngira video di pakuwon
bearing = 133.59

H = 45 #Ketinggian Pesawat (m)
...
def hitung():
    ...
    lat_target = degrees(nlat)
    lon_target = degrees(nlon)
    print('Latitude target            : %.7f' % lat_target)
    print('Longitude target           : %.7f' % lon_target)
    print('Jarak ke Koordinat Baru (m): '+str(jarak_x*1000))
...
cap = ImageTk.PhotoImage(Image.open("pakuwon.png"))
...
def show_frame():
    ...
    global lonl
    #lonl = lonl - 0.0001
    info_text = 'INFORMASI\n=========\nKetinggian                 : '+str(H)+'\nArah (Utara 0 Derajat)     : '+str(bearing)+'\nSudut Kamera               : '+str(Sudut)+'\nLatitude UAV               : %.7f\nLongitude UAV              : %.7f\n-----\nLatitude target            : %.7f\nLongitude target           : %.7f\nJarak ke Koordinat Baru (m): %.7f' % (latl, lonl, lat_target, lon_target,(jarak_x*1000))
    lmain_info.configure(text = info_text, font=("Courier", 15))    
